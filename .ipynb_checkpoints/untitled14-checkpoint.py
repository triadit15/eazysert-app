...
def upload_pdf():
    ...
    try:
        ...
        if found_id and found_id in verified_ids:
            ...
            certified_pdf = certified_image.replace(".jpg", ".pdf")
            image_to_pdf(certified_image, certified_pdf)

        return render_template('result.html', text=extracted_text,
                               image_path=certified_image,
...
def download_certified():
    certified_pdf = os.path.join(OUTPUT_FOLDER, 'page1_certified.pdf')
    return send_file(certified_pdf, as_attachment=True)
